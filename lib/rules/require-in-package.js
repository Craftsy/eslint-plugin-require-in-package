	...
	return {
		"ImportDeclaration": function(node) {
			...
			if (packagePath) {
				...
				var dependencyToValidate;
				if (dependencyName.indexOf('/') === -1) {
					dependencyToValidate = dependencyName;
				} else {
					if (dependencyName.indexOf('@') === 0) {
						var secondSlashIndex = dependencyName.indexOf('/', dependencyName.indexOf('/')+1);
						if (secondSlashIndex == -1) {
							dependencyToValidate = dependencyName;
						} else {
							dependencyToValidate = dependencyName.substr(0, secondSlashIndex);
						}
					} else {
						dependencyToValidate = dependencyName.substr(0, dependencyName.indexOf('/'));
					}
				}
